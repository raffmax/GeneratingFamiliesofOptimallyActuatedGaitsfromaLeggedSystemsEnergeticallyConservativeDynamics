...
%
function [f, v, c] = GetMainBodyMonopodPatch(colors,edges,epsilon)
    ...
    v =  TransformVertices(v,diag([0.7*epsilon+0.35*(1-epsilon),0.45,0.2]),[0,0,0]);                     
    c = ones(length(f),3)*diag(colors.torso);
    
%     if edges
%         v1 = TransformVertices(v,diag([0.705,0.4,0.205]),[0,0,0]); %(x,z,y) correct rectangle
%         c1 = zeros(length(f),3);
%         [v,f,c]=AddPatchesWithColor(v,f,c,v1,f,c1); 
%     end
    
	% Bent rotational cycliner (on which the hip actuation is attached) on
	% both sides: 
    [x, y] = cylinder(epsilon*[0; ones(40,1)*0.037; 0],20);
    z = repmat([0.8, 0.8, linspace(0.8, 0, 38),0,0]',1,21);
    [f1, v1] = surf2patch(x, y, z, z);
    % Set a uniform dark color

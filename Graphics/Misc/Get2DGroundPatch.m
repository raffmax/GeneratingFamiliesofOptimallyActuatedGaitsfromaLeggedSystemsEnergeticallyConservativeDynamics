...
function [f, v, c] = Get2DGroundPatch()
    ...
    v = [-5,0,0;
         ...
         repmat([w,0,-h],n,1) + [-5+linspace(0,s*n,n)',zeros(n,2)];
         -5+s*n+w,0,0];
    v(:,1) = -v(:,1);
    % Connect to faces:
    f = [1,2,4*n+1,4*n+2;
    ...
    % Create a white patch behind the ground pattern to hide the swing leg portion
    v1 = [-5,0,0;
          -5,0,-h;
          -5+(-ext+w),0,-(h+ext);
           5+(-ext+w),0,-(h+ext);
    ...
    [v, f, c] = AddPatchesWithColor(v1, f1, c1, v, f, c);    
